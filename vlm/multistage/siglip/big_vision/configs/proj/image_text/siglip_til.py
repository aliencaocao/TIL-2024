...
def get_config(arg=None):
    ...
    arg = bvcc.parse_arg(
        arg, res=384, runlocal=True, token_len=64, txt='bert_large', img='L/16',  # text token 64 is from https://huggingface.co/google/siglip-large-patch16-384 README
        init='gs://big_vision/siglip/webli_en_l16_384_63634585.npz', model_init='gs://big_vision/siglip/webli_en_l16_384_63634585.npz', img_head=False, batch_size=32)
    img_name, img_init = common.inits[arg.img]
    txt_name, txt_init = common.inits[arg.txt]
    config = ConfigDict()

    config.input = {}
    config.input.data = dict(name='til', split='train')
    ...
    epochs = 20
    data_len = 4068  # 80% train=4068 samples
    config.total_steps = 5_000 if not arg.runlocal else (int(data_len/config.input.batch_size) * epochs)

    config.init_shapes = [(1, arg.res, arg.res, 3), (1, arg.token_len,)]
    config.init_types = ['float32', 'int32']

    if arg.init:
        vocab_path = arg.init.rsplit('.', 1)[0] + '.txt'  # gs://big_vision/siglip/webli_en_l16_384_63634585.txt
    else:
    ...
    config.ckpt_steps = int(data_len/config.input.batch_size)  # org: 1000, 80% train=4068 samples, bs=32, 4068/32=127 steps/epoch
    ...
    dim = {'B': 768, 'L': 1024}[arg.img[0]]  # L here
    ...
    config.evals.retrieval_coco = common.get_coco(
        ...
        log_steps=config.ckpt_steps,  # org 1000, now we set same to per epoch
    )
